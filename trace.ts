...
export class TracePlugin {
  ...
  public panZoom(chart, increment) {
  ...
  public getOption(chart, category, name) {
  ...
  public afterEvent(chart, event) {
    ...
    if (!e.stop && syncEnabled) {
      let event = new CustomEvent('sync-event') as any;
  ...
  public resetZoom(chart, stop?: boolean, update?: boolean) {
    ...
    */
    ...
    if (!stop && update && syncEnabled) {
      ...
      var event = new CustomEvent('reset-zoom-event') as any;
      event.chartId = chart.id;
      event.syncGroup = syncGroup;
      window.dispatchEvent(event);
    }
  ...
  public interpolateValues(chart) {
    for (
      var chartIndex = 0;
      chartIndex < chart.data.datasets.length;
      chartIndex++
